<template lang="pug">
  .address
    .container
      .address-list(v-if='addressList.length > 0')
        .item(v-for='item in addressList', :key='item.id', @click='selectAddress(item.id)')
          .l
            .name {{item.name}}
            .default(v-if='item.is_default') 默认
          .c
            .mobile {{item.mobile}}
            .address {{item.full_region + item.address}}
          .r
            img.del(@click.stop='addressAddOrUpdate(item.id)', src='http://yanxuan.nosdn.127.net/hxm/yanxuan-wap/p/20161201/style/img/icon-normal/address-edit-7fee7b0d63.png')
...
export default {
  ...
  methods: {
    ...
    addressAddOrUpdate (addressId) {
      wx.navigateTo({
        url: '/pages/shopping/addressAdd?id=' + addressId
      })
    },
