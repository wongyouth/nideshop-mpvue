  ...
  .order-detail
    .container
      ...
      .order-bottom
        .address
          .t
            text.name {{orderInfo.consignee}}
            text.mobile {{orderInfo.mobile}}
          .b {{orderInfo.full_region + orderInfo.address}}
        .total
          .t
            text.label 商品合计：
            text.txt ￥{{orderInfo.goods_price}}
          .t
            text.label 运费：
            text.txt ￥{{orderInfo.freight_price}}
        .pay-fee
          text.label 实付：
          text.txt ￥{{orderInfo.actual_price}}
</template>
